...
class RegexReferenceDetector(ReferenceDetector):
    ...
    def _build_pattern() -> re.Pattern:
        ...
        single = fr"(?:article\s{number}{para}|this\s{node_name}|the previous\s{node_name}|{node_name_dec}\s{number}|{node_name_rom}\s{rom}|the\s{ordinal}\s{node_name}|that\s{node_name}|{document})"  # Missing thereof
