...
def test_detect():
    test_cases = [
        ...
        ("subsidiarity as set out in Article 5 of the Treaty on European Union", ["Article 5 of the Treaty on European Union"]),
    ...
    for text, result in test_cases:
        assert [x.text_content for x in matcher.detect(text)] == result
